  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 italic uppercase">COMMAND CENTER</h1>
          <p className="text-slate-400 font-medium mt-1 uppercase tracking-widest text-[10px] md:text-xs">
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <motion.div
              ...
              key={i}
              className="bg-white border border-slate-100 rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 shadow-xl shadow-slate-200/50 hover:scale-105 transition-transform"
            >
              <div className="flex items-center justify-between space-y-0 pb-4">
                <div className="p-2 bg-slate-50 rounded-xl"><Icon className="h-4 w-4 text-primary" /></div>
                <div className="text-[9px] md:text-[10px] font-black text-slate-300 uppercase tracking-widest">{stat.title}</div>
              </div>
              ...
              <p className="text-[9px] md:text-[10px] font-bold text-slate-400 mt-4 flex items-center gap-1 uppercase tracking-tighter">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-7 mt-8">
        ...
        <div className="bg-slate-900 text-white rounded-[2.5rem] lg:col-span-4 p-10 shadow-2xl relative overflow-hidden hidden md:block">
          <div className="relative z-10">
            <h3 className="text-xl font-black italic mb-2 tracking-tighter">REVENUE ANALYTICS</h3>
            ...
            <div className="h-[250px] w-full flex items-end justify-between gap-4 px-2 pb-4 pt-12 border-b border-white/5">
              {[40, 70, 45, 90, 65, 80, 110, 85, 95, 75, 120, 100].map((h, idx) => (
                ...
                >
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-white text-slate-900 text-[10px] font-black py-1.5 px-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-xl">
                    ₹{h}k
                  </div>
                </motion.div>
              ))}
        ...
        <div className="bg-white border border-slate-100 rounded-[2.5rem] lg:col-span-3 p-10 shadow-xl shadow-slate-200/50 flex flex-col h-full">
          <h3 className="text-xl font-black italic mb-2 tracking-tighter">RECENT ACTIVITY</h3>
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-10">Latest {enquiries?.data?.content?.length || 0} user inquiries</p>
          ...
          <div className="space-y-8 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {isLoading ? (
              ...
              <div key={enq.id} className="flex items-center group cursor-pointer">
                <div className="h-12 w-12 rounded-2xl bg-slate-50 flex items-center justify-center flex-shrink-0 text-primary font-black border border-slate-100 group-hover:bg-primary group-hover:text-white transition-colors uppercase">
                  {(enq.user?.email || '?').charAt(0)}
                </div>
                <div className="ml-5 space-y-1 overflow-hidden flex-1">
                  <p className="text-sm font-black text-slate-800 truncate leading-none capitalize">{enq.user?.name || enq.user?.email.split('@')[0]}</p>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Interest: {enq.serviceType}</p>
                </div>
      ...
      <div className="grid gap-8 md:grid-cols-2 mt-8">
          ...
          <div className="bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-xl shadow-slate-200/50">
            <div className="flex items-center justify-between mb-8">
                <div>
                   <h3 className="text-xl font-black italic tracking-tighter">UPCOMING TRIPS</h3>
                   <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Starting in next 14 days</p>
                </div>
          ...
          {/* Payment Reminders */}
          <div className="bg-[#FFF9F9] border border-red-50 rounded-[2.5rem] p-10 shadow-xl shadow-red-100/20">
            <div className="flex items-center justify-between mb-8">
                <div>
                   <h3 className="text-xl font-black italic tracking-tighter text-red-900">PAYMENT ALERTS</h3>
                   <p className="text-xs text-red-400 font-bold uppercase tracking-widest mt-1">Pending balances for confirmed bookings</p>
                </div>
...
function UpcomingBookingsList({ type }: { type: 'upcoming' | 'pending-payments' }) {
    ...
    return bookings.data.slice(0, 5).map((booking: any) => (
        <div key={booking.id} className="flex items-center justify-between p-4 bg-white/50 rounded-2xl border border-slate-100 hover:border-primary/20 transition-colors group">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h4 className="text-xs font-black text-slate-800 uppercase tracking-tight italic">{booking.customerName}</h4>
            ...
            <div className="flex flex-col items-end">
                <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${type === 'upcoming' ? 'bg-indigo-50 text-indigo-600' : 'bg-red-50 text-red-600'}`}>
