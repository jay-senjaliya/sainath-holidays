...
-- ============================================================
-- TOUR PACKAGES (6 packages)
-- ============================================================
INSERT INTO tour_packages (id, title, description, price, duration_days, latitude, longitude, location, category, is_active, created_by) VALUES
(1, 'Kerala Backwaters Bliss',
...
-- Correct sequences if we forced IDs
SELECT setval('tour_packages_id_seq', (SELECT MAX(id) FROM tour_packages));
...
-- VEHICLES
-- ============================================================
INSERT INTO vehicles (id, vehicle_type, name, description, price_per_day, seating_capacity, available, image_url) VALUES
(1, 'SUV',             'Toyota Innova Crysta',   'Premium 7-seater SUV, AC, music system, wide boot space.', 3500, 7, TRUE, 'https://images.unsplash.com/photo-1609521263047-f8f205293f24?w=800'),
(2, 'SEDAN',           'Honda City',             'Comfortable 4-seater sedan, AC, perfect for city tours.',  1800, 4, TRUE, 'https://images.unsplash.com/photo-1555215695-3004980ad54e?w=800'),
(3, 'TEMPO_TRAVELLER', 'Force Tempo Traveller',  '12-seater AC tempo, ideal for group pilgrimages and tours.', 5500, 12, TRUE, 'https://images.unsplash.com/photo-1570125909232-eb263c188f7e?w=800'),
(4, 'LUXURY',          'Mercedes E-Class',       'Premium luxury sedan with chauffeur service. Airport VIP pick-up.', 8000, 4, TRUE, 'https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?w=800')
...
-- HOTELS
-- ============================================================
INSERT INTO hotels (id, name, location, latitude, longitude, price_per_night, description, image_url, star_rating, is_active) VALUES
(1, 'The Leela Palace Udaipur', 'Udaipur, Rajasthan', 24.5854, 73.6826, 18999, 'A legendary royal palace hotel on the banks of Lake Pichola. Features infinity pool, spa, and multiple fine-dining restaurants.', 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=800', 5, TRUE),
(2, 'Zostel Manali', 'Manali, Himachal Pradesh', 32.2432, 77.1892, 1200, 'Vibrant backpacker hostel in the heart of Manali. Common room with mountain views, free WiFi, and adventure activity desk.', 'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?w=800', 2, TRUE),
(3, 'Taj Exotica Resort & Spa', 'Benaulim, Goa', 15.2673, 73.9348, 12500, 'Beachfront luxury resort in South Goa. Private beach, water sports, yoga pavilion, and 5-star dining.', 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=800', 5, TRUE)
ON CONFLICT (id) DO NOTHING;
